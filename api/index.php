...
<?php
    ...
    Api::POST("/user/login",function(){
        ...
        $data = database::query('SELECT id, username, admin_name FROM admin WHERE username = ? and password = ?', $_POST['username'], $password_hash);
        if (!empty($data)){
            $data['usertype']='admin';
        }else{
    ...
    // Api::GET("/check/user".Api::ALLSTRING,function($email){
    //     $data = database::query('SELECT email FROM member WHERE email = ?', $email);
    //     if (empty($data)){
    //         Api::send(['exist'=>FALSE]);
    //     }else{
    //         Api::send(['exist'=>TRUE]);
